...
class GetTickersInfoUseCaseTest {

    private class FakeTickerRepository {
        var infoResult: Map<Ticker, TickerData> = emptyMap()

        fun info(tickers: List<Ticker>): Map<Ticker, TickerData> {
            return infoResult.filterKeys { it in tickers }
        }
    }

    private class TestableGetTickersInfoUseCase(
        private val repository: FakeTickerRepository
    ) {
        operator fun invoke(tickers: List<Ticker>): Map<Ticker, TickerData> {
            return repository.info(tickers)
        }
    }

    @Test
    fun invokeCallsRepositoryInfo() {
        val repository = FakeTickerRepository()
        val useCase = TestableGetTickersInfoUseCase(repository)
        ...
        repository.infoResult = mapOf(ticker1 to data1, ticker2 to data2)
    ...
    @Test
    fun invokeReturnsEmptyMapWhenNoInfo() {
        val repository = FakeTickerRepository()
        val useCase = TestableGetTickersInfoUseCase(repository)
    ...
    fun invokeReturnsOnlyMatchingTickers() {
        ...
        repository.infoResult = mapOf(ticker1 to data1, ticker2 to data2, ticker3 to data3)
